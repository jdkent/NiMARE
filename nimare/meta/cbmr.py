...
class CBMREstimator(Estimator):
    ...
    def _model_structure(self, model, penalty, device):
        ...
        cbmr_model = self._models[model](
            beta_dim=beta_dim,
            gamma_dim=gamma_dim,
            groups=self.groups,
            study_level_moderators=study_level_moderators,
            penalty=penalty,
            device=device,
        )
        if "cuda" in device:
    ...
    def _fit(self, dataset):
        ...
        for group in self.inputs_["all_group_study_id"].keys():
            ...
            ] = group_studywise_spatial_intensity
        ...
        for group in self.inputs_["all_group_study_id"].keys():
            ...
            if self.model == "Poisson":
                ...
                nll = lambda beta: -GLMNB._log_likelihood_single_group(
                    alpha,
                    beta,
                    gamma,
                    Coef_spline_bases,
                ...
                nll = lambda beta: -GLMCNB._log_likelihood_single_group(
                    alpha,
                    beta,
                    gamma,
                    Coef_spline_bases,
...
<<<<<<< HEAD
                            Cov_group_log_intensity = np.concatenate(
                                (
                                    ...
                                    Cov_group_log_intensity_j.reshape(
                                        1,
                                    ),
                                ),
                                axis=0,
...
class GLMNB(torch.nn.Module):
    ...
    def _log_likelihood_single_group(
        ...
        log_l = GLMNB._three_term(group_foci_per_voxel, r, device=device) + torch.sum(
